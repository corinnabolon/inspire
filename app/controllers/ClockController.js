...
function _drawClock() {
  ...
  if (AppState.wantsTwentyFourHourClock == false) {
    ...
    let twentyFourHourSecondString = ""
    formattedTimeLowerNoSpace = formattedTimeLowerNoSpace.slice(2, 5)
    if (formattedTimeLowerNoSpace.includes("am")) {
      twentyFourHourSecondString = formattedTimeLowerNoSpace.replace("am", " ")
    } else {
      twentyFourHourSecondString = formattedTimeLowerNoSpace.replace("pm", " ")
    }
    if (newClock.hours.toString()[0] == "1" || newClock.hours.toString()[0] == "2" && newClock.hours.toString().length >= 2) {
